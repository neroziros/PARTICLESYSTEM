...
void Particleclass::Update(float elapsedSeconds)
{
	...
	// Command the "render" operation. This will translate in the actual particles EMISSION
	D3DX11_TECHNIQUE_DESC techDesc;
	g_pAdvanceParticles->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		g_pEmmitParticles->GetPassByIndex(p)->Apply(0, m_D3D->GetDeviceContext());

		// Execute the emission several times
		for (int i = 0; i < (float)emissionRate/128.0f; i++)
		{
			g_pfGlobalTime->SetFloat(m_TotalTimeElapsed + i);
	...
	D3DX11_TECHNIQUE_DESC techDesc0;
	g_pAdvanceParticles->GetDesc(&techDesc0);
	for (UINT p = 0; p < techDesc0.Passes; ++p)
	{
...
bool Particleclass::InitializeShader(ID3D11Device* device, HWND hwnd,LPCSTR fxFilename)
{
	...
	// NEW PARTICLE SYSTEM
	g_pEmmitParticles = pEffect->GetTechniqueByName("EmmitParticles");
	if (!g_pEmmitParticles->IsValid())return false;
...
bool Particleclass::Initialize(HWND hwnd)
{
	...
	*/
	// NEW PARTICLE SYSTEM
	if (!InitializeShader(m_D3D->GetDevice(), hwnd, "ParticleEffect_ALT.fx"))
		return false;
